...
namespace MSVenta.Venta.Services
{
    public class ProductoAlmacenService
    {
                ...
                public async Task<ProductoAlmacen> AddAsync(ProductoAlmacen productoAlmacen)
        {
            ...
            if (item == null)
            {
                throw new KeyNotFoundException($"No se encontr� un producto/insumo con ID {productoAlmacen.ItemId}.");
            }

            if (almacen == null)
            {
                throw new KeyNotFoundException($"No se encontr� un almac�n con ID {productoAlmacen.AlmacenId}.");
            }

            // Validar compatibilidad de tipos
            if (almacen.Tipo == "Productos" && item.Tipo != "Producto")
            {
                throw new InvalidOperationException("Este almac�n solo admite productos terminados.");
            }
            if (almacen.Tipo == "Insumos" && item.Tipo != "Insumo")
            {
                throw new InvalidOperationException("Este almac�n solo admite insumos (materia prima).");
            }

            // Verificar si la combinaci�n ItemId - AlmacenId ya existe
            ...
            if (existe)
            {
                throw new InvalidOperationException("El producto/insumo ya est� asignado a este almac�n.");
            }
                ...
                public async Task<List<ProductoAlmacen>> AddBulkAsync(int almacenId, List<ItemStockDto> itemsDto)
        {
            ...
            if (almacen == null)
            {
                throw new KeyNotFoundException($"No se encontr� un almac�n con ID {almacenId}.");
            }
            ...
            foreach (var itemDto in itemsDto)
            {
                ...
                if (item == null)
                {
                    throw new KeyNotFoundException($"No se encontr� un item con ID {itemDto.ItemId}.");
                }

                // Validar compatibilidad de tipos
                if (almacen.Tipo == "Productos" && item.Tipo != "Producto")
                {
                    throw new InvalidOperationException($"El item '{item.Nombre}' no se puede asignar. Este almac�n solo admite productos terminados.");
                }
                if (almacen.Tipo == "Insumos" && item.Tipo != "Insumo")
                {
                    throw new InvalidOperationException($"El item '{item.Nombre}' no se puede asignar. Este almac�n solo admite insumos (materia prima).");
                }
